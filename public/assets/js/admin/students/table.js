...
export function loadStudentCard() {
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      document.getElementById("total-students").textContent = Number(
        counts.active || 0,
      );
      document.getElementById("total-active").textContent = Number(
        counts.active || 0,
      );
      document.getElementById("total-inactive").textContent = Number(
        counts.inactive || 0,
      );
